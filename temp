...
const HomeCard = ({ header, data, onCardPress, activeCard, setData }) => {
  const scrollViewRef = useRef(null);
  const [currentActiveCard, setCurrentActiveCard] = useState(activeCard);
  const navigation = useNavigation();

  const cardWidth = screenWidth * 0.3; // Card width as 30% of screen width
  const cardMargin = 10; // Card margin
  const centerOffset = (screenWidth - cardWidth) / 2;

  // Update active card on prop change
  useEffect(() => {
  ...
  const getCardBackgroundColor = (index) => {
    const colors = ['#FFF8DC', '#E6F2FF', '#E6FFE6', '#F0E6FF']; // Define colors centrally if needed
      ...
      >
        {data.map((item, index) => (
          <View key={item.name} style={styles.cardWrapper}>
            <TouchableOpacity
              onPress={() => onCardPress(item)} // Pass the selected item
              style={[
                styles.card,
...
const styles = StyleSheet.create({
  ...
  cardsContainer: {
    ...
    paddingVertical: screenWidth * 0.05, // Remove padding to ensure centering
  },
  cardWrapper: {
    marginHorizontal: 10,
  ...
  plate: {
    width: screenWidth * 0.22, // Dynamic width
    height: screenWidth * 0.22, // Dynamic height
    borderRadius: screenWidth * 0.11, // Dynamic border radius
    borderWidth: 2,
    borderColor: '#D3D3D3',
    justifyContent: 'center',
    alignItems: 'center',
  ...
  cardImage: {
    ...
    resizeMode: 'cover',
    borderRadius: screenWidth * 0.1, // Dynamic border radius
  },
  iconContainer: {
    position: 'absolute',
    top: 10,
